 ...
 */
public final class FrDoubleMetaphoneFilter extends TokenFilter {
	 ...
	 *  replacing them.
	 */
	public FrDoubleMetaphoneFilter(final TokenStream input, final int maxCodeLength, final boolean inject, final boolean withFr, final boolean withMetaphone) {
		super(input);
	...
	public boolean incrementToken() throws IOException {
		for (;;) {
			...
			final String primaryPhoneticValue = encoder.doubleMetaphone(v);
			final String alternatePhoneticValue = encoder.doubleMetaphone(v, true);
			final String frenchPhoneticValue = Phonetic.genererPhonetic(v);
			...
			if (withMetaphone && primaryPhoneticValue != null && primaryPhoneticValue.length() > 0 && !primaryPhoneticValue.equals(v)) {
				if (saveState) {
			...
			if (withMetaphone && alternatePhoneticValue != null && alternatePhoneticValue.length() > 0 && !alternatePhoneticValue.equals(primaryPhoneticValue) && !primaryPhoneticValue.equals(v)) {
				if (saveState) {
					remainingTokens.addLast(captureState());
					saveState = false;
				}
			...
			if (withFr && frenchPhoneticValue != null && frenchPhoneticValue.length() > 0 && !frenchPhoneticValue.equals(primaryPhoneticValue) && !frenchPhoneticValue.equals(alternatePhoneticValue) && !frenchPhoneticValue.equals(v)) {
				if (saveState) {
					remainingTokens.addLast(captureState());
					saveState = false;
				}
				posAtt.setPositionIncrement(firstAlternativeIncrement);
				termAtt.setEmpty().append(frenchPhoneticValue);
				saveState = true;
			}
