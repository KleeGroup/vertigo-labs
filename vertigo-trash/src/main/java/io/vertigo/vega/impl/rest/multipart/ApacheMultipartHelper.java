...
public final class ApacheMultipartHelper {
	...
	private static void wrapParameters(final HttpServletRequest httpServletRequest, final Map<String, List<String>> parameters, final Map<String, VFile> uploadedFiles, final Map<String, RuntimeException> tooBigFiles) throws FileUploadException, IOException {
		...
		// Param�trage de la limite par fichier (celle sur la taille de la request n'est pas interceptable)
		...
		while (iterator.hasNext()) {
			...
			} else if (fileItem.getName().length() > 0) { // Si le nom est vide, le champ n'�tait pas renseign�.
				// Si le fichier est vide alors soit le nom ne correspond pas � un fichier, soit il �tait vide cot� cot�
				// client
