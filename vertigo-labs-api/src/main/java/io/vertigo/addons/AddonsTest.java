...
public class AddonsTest extends AbstractTestCaseJU4 {
	...
	public void testAccounts() {
		final Account account0 = new AccountBuilder("0")
				...
				.build();
		accountManager.getStore().saveAccount(account0);

		final Account account1 = new AccountBuilder("1")
				.withDisplayName("hector")
				.build();
		accountManager.getStore().saveAccount(account1);

		final Account account2 = new AccountBuilder("2")
				.withDisplayName("Priam")
				.build();
		accountManager.getStore().saveAccount(account2);

		Assert.assertEquals("zeus", accountManager.getStore().getAccount(accountURI0).getDisplayName());

		Assert.assertEquals(3, accountManager.getStore().getAllAccounts().size());
	}
	...
	public void testGroups() {
		...
		accountManager.getStore().saveGroup(group);
		...
		Assert.assertEquals(0, accountManager.getStore().getGroupURIs(accountURI1).size());
		...
		Assert.assertEquals(0, accountManager.getStore().getGroupURIs(accountURI0).size());
