...
public final class DslDynamicRegistryMock implements DynamicRegistry {
	...
	public EntityGrammar getGrammar() {
		return PersonGrammar2.GRAMMAR;
	}

	//private final List<DynamicDefinition> dynamicDefinitions = new ArrayList<>();

	@Override
	public Option<Definition> createDefinition(final DefinitionSpace definitionSpace, final DynamicDefinition dynamicDefinition) {
		final Object definition = DslSmartEntityBuilder.build(dynamicDefinition);
		//	dynamicDefinitions.add((Definition) definition);
		return Option.none();
	}
